...
let AiService = class AiService {
    ...
    async generateThesisContent(thesisData) {
        const { title, authorFirstName, authorLastName, advisorFirstName, advisorLastName, advisorDegree, researchLine, city, year, } = thesisData;
        const prompt = `
...
Las limitaciones del estudio en el aspecto espacial están delimitadas al entorno geográfico de la ciudad de ${city}, Región La Libertad, Perú, y no podrán extrapolarse automáticamente a otros contextos sin una validación adicional. En el aspecto temporal, la investigación comprende el período correspondiente al año ${year}, por lo que los resultados reflejan las condiciones tecnológicas, organizacionales y sociales vigentes en dicho período.`,
    ...
    async generateThesisPdf(thesisData, userEmail) {
        try {
            const content = await this.generateThesisContent(thesisData);
            return new Promise((resolve, reject) => {
                try {
                    const marginLeft = 85;
                    const marginOther = 71;
                    ...
                    if (content.referencias && content.referencias.length > 0) {
                        content.referencias.forEach((ref) => {
                ...
                if (userEmail && pdfBuffer) {
                    try {
                        await this.sendThesisEmail(userEmail, thesisData.title, pdfBuffer, thesisData.authorFirstName);
                    }
                    catch (emailError) {
    ...
    async generateThesisWord(thesisData, userEmail) {
        const content = await this.generateThesisContent(thesisData);
        const makeHeading1 = (text) => new docx_1.Paragraph({ text, heading: docx_1.HeadingLevel.HEADING_1, spacing: { before: 400, after: 200 } });
        const makeHeading2 = (text) => new docx_1.Paragraph({ text, heading: docx_1.HeadingLevel.HEADING_2, spacing: { before: 200, after: 100 } });
        const makeBody = (text) => new docx_1.Paragraph({
            text: text || 'No disponible.',
            alignment: docx_1.AlignmentType.JUSTIFIED,
            spacing: { after: 200, line: 480 },
        });
        const makePageBreak = () => new docx_1.Paragraph({ children: [new docx_1.PageBreak()] });
            ...
            sections: [
                {
                    properties: {},
                    children: [
                        new docx_1.Paragraph({ text: 'UNIVERSIDAD NACIONAL DE TRUJILLO', heading: docx_1.HeadingLevel.HEADING_1, alignment: docx_1.AlignmentType.CENTER, spacing: { after: 50 } }),
                        new docx_1.Paragraph({ text: 'FACULTAD DE INGENIERÍA', heading: docx_1.HeadingLevel.HEADING_2, alignment: docx_1.AlignmentType.CENTER, spacing: { after: 50 } }),
                        new docx_1.Paragraph({ text: 'Escuela Profesional de Ingeniería de Sistemas', alignment: docx_1.AlignmentType.CENTER, spacing: { after: 400 } }),
                        new docx_1.Paragraph({ text: thesisData.title, alignment: docx_1.AlignmentType.CENTER, spacing: { after: 100 } }),
                        new docx_1.Paragraph({ text: 'PROYECTO DE TESIS', heading: docx_1.HeadingLevel.HEADING_1, alignment: docx_1.AlignmentType.CENTER, spacing: { after: 400 } }),
                        new docx_1.Paragraph({ children: [new docx_1.TextRun({ text: 'AUTOR(ES):', bold: true, font: 'Arial Narrow', size: 24 })], spacing: { after: 50 } }),
                        new docx_1.Paragraph({ text: `${thesisData.authorLastName} ${thesisData.authorFirstName}`, spacing: { after: 100 } }),
                        new docx_1.Paragraph({ children: [new docx_1.TextRun({ text: 'ASESOR:', bold: true, font: 'Arial Narrow', size: 24 })], spacing: { after: 50 } }),
                        new docx_1.Paragraph({ text: `${thesisData.advisorDegree} ${thesisData.advisorLastName} ${thesisData.advisorFirstName}`, spacing: { after: 100 } }),
                        new docx_1.Paragraph({ children: [new docx_1.TextRun({ text: 'LÍNEA DE INVESTIGACIÓN:', bold: true, font: 'Arial Narrow', size: 24 })], spacing: { after: 50 } }),
                        new docx_1.Paragraph({ text: thesisData.researchLine, spacing: { after: 600 } }),
                        new docx_1.Paragraph({ text: `${(thesisData.city || 'TRUJILLO').toUpperCase()} - ${thesisData.year}`, alignment: docx_1.AlignmentType.CENTER, spacing: { after: 0 } }),
                        makePageBreak(),
                        ...
                        ...(content.referencias && content.referencias.length > 0
                            ? content.referencias.map((ref) => makeBody(`${ref.cita}`))
    ...
    async sendThesisEmail(userEmail, thesisTitle, pdfBuffer, studentName) {
    ...
    async chatWithThesisAgent(message, sessionId) {
